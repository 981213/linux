				  ...
				  unsigned long id)
{
	...
	shift = sf21a6826_reset_shift(id);
	mask = BIT(shift);
	return regmap_update_bits(rd->regmap, SF21A6826_SOFT_RESET,
				mask, 0);
}
			       ...
			       unsigned long id)
{
	...
	rd = container_of(rcdev, struct sf21a6826_reset_data, rcdev);
	ret = regmap_read(rd->regmap, SF21A6826_SOFT_RESET, &reg);
	if (ret)
		return ret;
...
static int sf21a6826_reset_probe(struct platform_device *pdev)
{
	...
	rd->rcdev.nr_resets = SF21A6826_RESET_MAX + 1;
