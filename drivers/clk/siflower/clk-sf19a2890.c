...
#if 0
//TODO: implement pll set rate
static void sf_pll_param_set(struct sf19a2890_clk *priv, u32 val, u32 hi,
			     u32 lo)
{
	struct sf_clk_common *cmn = &priv->common;
	int reg_hi = hi / 8;
	...
	if (reg_hi == reg_lo) {
		u32 mask = BIT(reg_hi_pos - reg_lo_pos + 1) - 1;
		sf_rmw(cmn, priv->offset + REG_PLL_PARAM(reg_hi),
		       mask << reg_lo_pos, (val & mask) << reg_lo_pos);
	} else {
		u32 lo_bits = 7 - reg_lo_pos + 1;
		u32 mask = BIT(lo_bits) - 1;
		sf_rmw(cmn, priv->offset + REG_PLL_PARAM(reg_lo),
		       mask << reg_lo_pos, (val & mask) << reg_lo_pos);
		val >>= lo_bits;

		for (i = reg_lo + 1; i < reg_hi; i++) {
			sf_writel(cmn, priv->offset + REG_PLL_PARAM(i),
				  val & 0xff);
			val >>= 8;
		}

		mask = BIT(reg_hi_pos + 1) - 1;
		sf_rmw(cmn, priv->offset + REG_PLL_PARAM(reg_hi), mask,
		       val & mask);
	}
}
#endif
					     ...
					     unsigned long parent_rate)
{
	...
	if (!(pll_pd & PLL_PD_DSM)) {
		u32 frac = sf_pll_param_get(priv, PLL_FRAC_HI, PLL_FRAC_LO);
		u64 frac_rate = ((u64)rate * frac) >> PLL_FRAC_BITS;
		rate += frac_rate;
	}
...
static const char *clk_pll_parents[] = { "osc12m", "osc40m" };
						...
						unsigned long parent_rate)
{
	struct sf_clk_common *cmn_priv = hw_to_sf_clk_common(hw);
	struct sf19a2890_clk *priv = cmn_to_clk(cmn_priv);
	u32 div = sf_readl(cmn_priv, priv->offset + REG_MUXDIV_RATIO) + 1;
...
static const char *clk_muxdiv_parents[] = { "pll_cpu", "pll_ddr", "pll_cmn",
					    "osc12m", "osc40m" };
...
static SF19A28_MUXDIV(muxdiv_mem_phy, 0x140, 0);
